...
TOP_MARGIN="$(sh ~/scripts/functions -getMargin top)"
BOTTOM_MARGIN="$(sh ~/scripts/functions -getMargin bottom)"
LEFT_MARGIN="$(sh ~/scripts/functions -getMargin left)"
RIGHT_MARGIN="$(sh ~/scripts/functions -getMargin right)"
TITLE_BAR_HEIGHT=20 #works best if all the windows are border less and this is set to zero

#either set these in a file or calculate them from the screen properties
MASTER_WIDTH="$(xwininfo -root | grep 'Width:' | awk '{print $2/2}')"
MASTER_HEIGHT="$(xwininfo -root | grep 'Height:' | awk '{print $2/2}')"
...
"LEFT_MASTER")
            ...
            wmctrl    -r :ACTIVE: -e "0,$X,$(( Y +20 )),$W,$(( H -20 ))"
             ...
             W=$((WIDTH - MASTER_WIDTH -USELESS_GAPS - RIGHT_MARGIN))
